...
export default function CircuitPageContent({ slug }: CircuitPageContentProps) {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const convexApi = api as any;
  const circuit = useQuery(convexApi.standings.getCircuitBySlug, { slug });

  if (circuit === undefined) {
    return (
      <main className="max-w-6xl px-2 lg:px-10 mx-auto mt-14 md:mt-0 w-full overflow-x-hidden font-onest">
        <SectionContainer className="flex items-center justify-center py-24">
          <p className="text-neutral-300">Loading circuit...</p>
        </SectionContainer>
      </main>
    );
  }
  ...
  if (circuit === null) {
    return (
      <main className="max-w-6xl px-2 lg:px-10 mx-auto mt-14 md:mt-0 w-full overflow-x-hidden font-onest">
        <SectionContainer className="flex items-center justify-center py-24">
          <p className="text-neutral-300">Circuit not found.</p>
        </SectionContainer>
      </main>
    );
  }
  ...
  return (
    <main className="max-w-6xl px-2 lg:px-10 mx-auto mt-14 md:mt-0 w-full overflow-x-hidden font-onest">
      ...
      <SectionContainer className="flex flex-col items-center justify-center pb-20 gap-y-20">
        <div className="flex flex-row items-center justify-center w-full text-center">
          <div className="h-[4px] w-1/5 bg-gradient-to-r from-transparent via-neutral-100/60 to-transparent"></div>
          <h2 className="text-neutral-200 font-semibold text-4xl w-3/5 font-f1-bold">
            {circuit.circuit}
          </h2>
          <div className="h-[4px] w-1/5 bg-gradient-to-r from-transparent via-neutral-100/60 to-transparent"></div>
        </div>
        <div className="flex flex-col md:flex-row items-center justify-center gap-x-12 gap-y-14">
          <div className="relative flex items-center justify-center group">
            <img
              src={circuit.circuitImage}
              alt={circuit.name}
              className="h-64 md:h-74 object-contain blur-sm opacity-50 transition duration-500 ease-in-out group-hover:scale-[1.3] delay-200 group-hover:opacity-0"
            />
          ...
          <div className="flex flex-col gap-y-8 md:grid md:grid-cols-[repeat(2,1fr)] md:grid-rows-[repeat(3,auto)] md:gap-x-6 md:gap-y-8 font-f1-regular text-neutral-100 cursor-default">
            <div className="flex flex-col items-start justify-center gap-y-2 rounded-xl py-2 px-4 backdrop-blur-md bg-gradient-to-b from-[#15122c] via-[#15122c]/50 to-transparent border-0 transition-transform duration-500 ease-in-out hover:-translate-y-2">
              <h2 className="font-normal text-xl text-neutral-200">First Grand Prix</h2>
              <h3 className="font-semibold text-3xl">{circuit.firstGrandPrix}</h3>
            </div>

            <div className="flex flex-col items-start justify-center gap-y-2 rounded-xl py-2 px-4 backdrop-blur-md bg-gradient-to-b from-[#15122c] via-[#15122c]/50 to-transparent border-0 transition-transform duration-500 ease-in-out hover:-translate-y-2">
              <h2 className="font-normal text-xl text-neutral-200">Number of Laps</h2>
              <h3 className="font-semibold text-3xl">{circuit.numberOfLaps}</h3>
            </div>

            <div className="flex flex-col items-start justify-center gap-y-2 rounded-xl py-2 px-4 backdrop-blur-md bg-gradient-to-b from-[#15122c] via-[#15122c]/50 to-transparent border-0 transition-transform duration-500 ease-in-out hover:-translate-y-2">
              <h2 className="font-normal text-xl text-neutral-200">Circuit Length</h2>
              <h3 className="font-semibold text-3xl">{circuit.circuitLength}</h3>
            </div>

            <div className="flex flex-col items-start justify-center gap-y-2 rounded-xl py-2 px-4 backdrop-blur-md bg-gradient-to-b from-[#15122c] via-[#15122c]/50 to-transparent border-0 transition-transform duration-500 ease-in-out hover:-translate-y-2">
              <h2 className="font-normal text-xl text-neutral-200">Race Distance</h2>
              <h3 className="font-semibold text-3xl">{circuit.raceDistance}</h3>
            </div>

            <div className="flex flex-col items-start justify-center gap-y-2 md:col-span-2 rounded-xl py-2 px-4 backdrop-blur-md bg-gradient-to-b from-[#15122c] via-[#15122c]/50 to-transparent border-0 transition-transform duration-500 ease-in-out hover:-translate-y-2">
              <h2 className="font-normal text-xl">Fastest Lap</h2>
              <h3 className="font-semibold text-3xl gap-2">
                {circuit.lapRecord}
                <span className="font-semibold text-2xl">{circuit.driverLapRecord}</span>
              </h3>
            </div>
          </div>
        </div>
